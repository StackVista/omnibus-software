...
build do
  ...
  if ohai["platform_family"] == "debian"
    command "curl -L -O http://dd-agent.s3.amazonaws.com/python-rrdtool/deb/#{ohai['kernel']['machine']}/rrdtool.so", :cwd => "#{install_dir}/embedded/lib/python2.7/"
  elsif ohai["platform_family"] == "rhel"
    command "curl -L -O http://files.directadmin.com/services/9.0/ExtUtils-MakeMaker-6.31.tar.gz", :cwd => "/tmp/"
    ...
    if ohai["kernel"]["machine"] == "i686"
      command "curl -L -O http://cpansearch.perl.org/src/JHI/perl-5.8.0/thrdvar.h", :cwd => "/usr/lib/perl5/CORE/"
    end
    command "make", :cwd => "/tmp/ExtUtils-MakeMaker-6.31"
    command "make install", :cwd => "/tmp/ExtUtils-MakeMaker-6.31"
    command "curl -k -L -O https://oss.oetiker.ch/rrdtool/pub/rrdtool-#{version}.tar.gz", :cwd => "/opt/"
